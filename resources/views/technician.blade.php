...
<head>
    ...
    <title>Technician Dashboard - E-Cooking Inventory</title>
    ...
    <nav class="navbar navbar-dark fixed-top" style="background: linear-gradient(90deg, #140168 0%, #5039d6 100%); z-index: 9999; padding: 0.2rem 0.3rem; min-height: 45px;">
        <div class="container-fluid d-flex align-items-center justify-content-between" style="gap: 0.2rem; flex-wrap: nowrap;">
            ...
            <div class="d-none d-md-block" style="flex-grow: 1; text-align: center;">
                <span class="text-white" style="font-size: 0.9rem;"><i class="fas fa-user-cog"></i> Technician Dashboard</span>
            </div>
            <div class="d-flex align-items-center" style="gap: 0.2rem; flex-shrink: 0; flex-wrap: nowrap;">
                <div class="text-white text-center" style="font-size: 0.65em; padding: 2px 4px; background: rgba(255,255,255,0.1); border-radius: 3px;">
                    <span id="technicianTimeVal"></span> <span id="technicianDate"></span>
                </div>
                <div class="dropdown">
                    <button class="btn btn-outline-light dropdown-toggle" type="button" data-bs-toggle="dropdown" style="padding: 0.2rem 0.4rem; font-size: 0.8rem; display: flex; align-items: center; gap: 0.25rem;">
                        @if(auth('technician')->user()->profile_picture)
                            <img src="{{ asset('storage/' . auth('technician')->user()->profile_picture) }}" alt="Profile" class="rounded-circle" style="width: 24px; height: 24px; object-fit: cover;">
                        @else
                            <i class="fas fa-user"></i>
                        @endif
                        <span class="d-none d-md-inline">Welcome back {{ auth('technician')->user()->name }}</span>
                    </button>
                    <ul class="dropdown-menu dropdown-menu-end shadow-lg">
                        <li class="px-3 py-2 border-bottom">
                            <small class="text-muted">Signed in as</small>
                            <div class="fw-bold">{{ auth('technician')->user()->name }}</div>
    ...
    <div class="offcanvas offcanvas-start" tabindex="-1" id="sidebar" aria-labelledby="sidebarLabel" data-bs-scroll="true" data-bs-backdrop="false">
        <div class="offcanvas-header d-lg-none border-bottom">
            <h5 class="offcanvas-title fw-bold" id="sidebarLabel">
                <i class="fas fa-user-cog text-info me-2"></i>Technician
            </h5>
        ...
        <div class="offcanvas-body d-lg-block d-flex flex-column" style="padding: 1rem; padding-top: 50px;">
            <div class="text-center mb-3">
                <div class="sidebar-header p-3 mb-2">
                    @if(auth('technician')->user()->profile_picture)
                        <img src="{{ asset('storage/' . auth('technician')->user()->profile_picture) }}" alt="Profile" class="rounded-circle mb-2" style="width: 60px; height: 60px; object-fit: cover; border: 3px solid white;">
                    @else
                        <div class="bg-white rounded-circle p-2 d-inline-flex mb-2">
                            <i class="fas fa-user fa-2x text-info"></i>
                        </div>
                    @endif
                    <h6 class="text-white fw-bold mb-1">{{ auth('technician')->user()->name }}</h6>
                    <div class="technician-badge">Technician Dashboard</div>
                </div>
            </div>
            ...
            <div class="sidebar-footer mt-auto">
                ...
                <nav class="nav flex-column gap-1">
                    <a class="nav-link text-success" href="/technician/panel">
                        <i class="fas fa-user-circle"></i>Panel
                    </a>
                    <a class="nav-link text-success" href="/technicians">
    ...
    <main class="main-content" style="padding-top: 60px; padding-bottom: 100px; position: relative; z-index: 1;">
        <div class="container-fluid" style="padding-top: 0; margin-top: 0;">
            ...
            <div class="row g-4 mt-3">
                <!-- Inventory Stats -->
                <div class="col-12" id="inventory-stats">
                    <div class="card">
                        <div class="card-header" style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white;">
                            <div class="d-flex justify-content-between align-items-center">
                                <h5 class="mb-0"><i class="fas fa-cogs me-2"></i>Parts Inventory Management</h5>
    ...
    <script>
        ...
        (function() {
            function init() {
                ...
                setInterval(updateTechnicianTime, 1000);
            }
        ...
        function updateTechnicianTime() {
            const now = new Date();
            document.getElementById('technicianTimeVal').textContent = now.toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' });
            document.getElementById('technicianDate').textContent = now.toLocaleDateString('en-US', { month: 'short', day: 'numeric' });
        }
        ...
        function renderPartsGrid(parts) {
            ...
            parts.forEach(part => {
                const price = part.price ? `UGX ${parseFloat(part.price).toLocaleString()}` : 'N/A';
                const imgSrc = part.image_path ? '/storage/' + part.image_path : 'https://via.placeholder.com/150?text=No+Image';
                const partNum = part.part_number || 'N/A';
                const appType = part.appliance ? part.appliance.name : 'N/A';
                const badge = part.appliance ? part.appliance.color : 'bg-secondary';
                html += `<div class="col-md-4 col-lg-3">
                    <div class="card h-100 shadow-sm">
                        ...
                        <div class="card-body">
                            ...
                            <p class="text-muted small mb-2">${partNum}</p>
                            <span class="badge ${badge} mb-2">${appType}</span>
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-8">
                                    ...
                                    <div class="row mb-2">
                                        ...
                                        <div class="col-6">${part.price ? 'UGX ' + parseFloat(part.price).toLocaleString() : 'N/A'}</div>
                                    </div>
                                    <div class="row mb-2">
                                    ...
                                    ${part.appliances && part.appliances.length ? `<div class="row mb-2"><div class="col-6"><strong>Compatible With:</strong></div><div class="col-6">${part.appliances.join(', ')}</div></div>` : ''}
                                </div>
                            </div>
        ...
        function renderAppliancesGrid(appliances) {
            ...
            appliances.forEach(app => {
                const statusClass = app.status === 'Available' ? 'success' : (app.status === 'In Use' ? 'info' : 'danger');
                const price = app.price ? `UGX ${parseFloat(app.price).toLocaleString()}` : 'N/A';
                const imgSrc = app.image_path ? '/storage/' + app.image_path : 'https://via.placeholder.com/150?text=' + encodeURIComponent(app.name);
                const brandName = app.brand ? app.brand.name : 'N/A';
                html += `<div class="col-md-4 col-lg-3">
                    <div class="card h-100 shadow-sm">
                        ...
                        <div class="card-body">
                            ...
                            <p class="text-muted small mb-2">${brandName}</p>
        ...
        function viewApplianceDetails(app) {
            const imgSrc = app.image_path ? '/storage/' + app.image_path : 'https://via.placeholder.com/300?text=' + encodeURIComponent(app.name);
            const statusClass = app.status === 'Available' ? 'success' : (app.status === 'In Use' ? 'info' : 'danger');
            const brandName = app.brand ? (app.brand.name || app.brand) : 'N/A';
                    ...
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <div class="row">
                                ...
                                <div class="col-md-8">
                                    <h5>${app.name}</h5>
                                    <p class="text-muted">${brandName}</p>
                                    <hr>
                                    <div class="row mb-2">
                                    ...
                                    <div class="row mb-2">
                                        <div class="col-6"><strong>Price:</strong></div>
                                        <div class="col-6">${app.price ? 'UGX ' + parseFloat(app.price).toLocaleString() : 'N/A'}</div>
                                    </div>
                                    <div class="row mb-2">
                                        <div class="col-6"><strong>Status:</strong></div>
